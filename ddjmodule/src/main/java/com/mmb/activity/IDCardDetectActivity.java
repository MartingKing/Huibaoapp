...
import com.mmb.utils.Configuration;
import com.mmb.utils.ICamera;
import com.mmb.utils.ModelUtil;
import com.mmb.utils.RotaterUtil;
import com.mmb.utils.StatusBarCompat;
import com.mmb.view.IDCardGuide;
import com.mmb.view.IDCardGuideH;
...
public class IDCardDetectActivity extends Activity implements TextureView.SurfaceTextureListener, Camera.PreviewCallback, View.OnClickListener {
    ...
    private void initView() {
        ...
        mRelativeLayout = findViewById(R.id.rl_idcard_cn_root_view);
        mRlMegviiGoBack = findViewById(R.id.rl_megvii_idcard_cn_goback);
        mRlMegviiGoBackH = findViewById(R.id.rl_megvii_idcard_cn_goback_h);
        mImageView = findViewById(R.id.iv_idcard_cn_image);
        mTextureView = findViewById(R.id.idcardscan_cn_layout_surface);
        mIvBottomLogoH = findViewById(R.id.iv_idcard_cn_bottom_logo_h);
        mRlMegviiTips = findViewById(R.id.rl_megvii_idcard_cn_tips);
        mTitleBar = findViewById(R.id.in_idcard_cn_title_bar);
        mTitleBarH = findViewById(R.id.rl_megvii_idcard_cn_title_bar_h);
        mTvMegviiTips = findViewById(R.id.tv_megvii_idcard_cn_tips);
        mTvMegviiTipsH = findViewById(R.id.tv_megvii_idcard_cn_tips_h);

        mIvPeopleIconlightView = findViewById(R.id.iv_people_light_icon);
        mIvChinaIconLightView = findViewById(R.id.iv_china_light_icon);

        mIbAnimalBreathView = findViewById(R.id.ib_animal_breath_view);
        mIbAnimalOneView = findViewById(R.id.ib_animal_one_view);

        mToastTitle = findViewById(R.id.toast_tv);
        mToastTip = findViewById(R.id.iv_auth_toast_tip);
        mLayoutToast = findViewById(R.id.layout_toast);

        mTvTipText = findViewById(R.id.tv_tip_text);
        mIvIconTickAll = findViewById(R.id.iv_icon_tick_all);
        mIdcardGuide = findViewById(R.id.idcardscan_cn_layout_guide);
        mIdcardGuideH = findViewById(R.id.idcardscan_cn_layout_guide_h);
    ...
    private void handleSuccessResult() {
        ...
        //不同手机，intent传值限制的大小不同，实际应用需要注意！此处仅做示例。实际应用可选择其它方式进行传递bitmap
    ...
    private void showFaileToast(IDCardQualityResult.IDCardResultType faileType) {
        ...
        try {
            ...
            if (!"".equals(toastStr)) {
                if (isTheSameError) {
                    showMyToast(toastStr, false);
                } else {
                    showMyToast(toastStr, true);
                }
                toastTime = System.currentTimeMillis();
            }
    ...
    private boolean isMyToastShown() {
        return View.VISIBLE == mLayoutToast.getVisibility();
    }
