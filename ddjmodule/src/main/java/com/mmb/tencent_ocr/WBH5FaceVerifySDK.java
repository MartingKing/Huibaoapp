...
public class WBH5FaceVerifySDK {
    ...
    public boolean recordVideoForApiBelow21(ValueCallback<Uri> uploadMsg, String acceptType, Activity activity) {
        if ("video/webank".equals(acceptType) || acceptType.contains("video")) {
    ...
    private void recordVideo(Activity activity) {
        List<String> curNeedPermissions = new ArrayList();
        if(Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
            for (int i = 0; i < permissions.length; i++) {
                if (ContextCompat.checkSelfPermission(activity,permissions[i])!=PackageManager.PERMISSION_GRANTED){
                    curNeedPermissions.add(permissions[i]);
                }
            }
            if (curNeedPermissions.size() == 0){
                jumToVideoPage(activity);
            }else {
                ActivityCompat.requestPermissions(activity,
                        curNeedPermissions.toArray(new String[curNeedPermissions.size()]),
                        TecentIdentityReconitionActivity.CAMERA_CODE);
            }
        }else {
            jumToVideoPage(activity);
        }
    ...
    public static void jumToVideoPage(Activity activity){
        try {
